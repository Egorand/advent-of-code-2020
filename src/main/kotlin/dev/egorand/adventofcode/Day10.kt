...
fun findAdapterSequence(joltages: List<Int>): Int {
  val joltages = joltages.sorted()
  val joltageDifferences = IntArray(3)
  joltageDifferences[joltages[0] - 1] += 1 // Diff between first joltage and 0.
  for (i in 1 until joltages.size) {
    joltageDifferences[joltages[i] - joltages[i - 1] - 1] += 1
  }
  joltageDifferences[2] += 1 // Highest joltage + 3 always gives a difference of 3.
  return joltageDifferences[0] * joltageDifferences[2]
}
