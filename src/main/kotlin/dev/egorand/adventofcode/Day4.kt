...
import dev.egorand.adventofcode.PassportDataValidator.Policy.FIELD_PRESENCE
import dev.egorand.adventofcode.PassportDataValidator.Policy.STRICT
...
fun validPassports(
  ...
  policy: PassportDataValidator.Policy,
): List<PassportData> {
  val valid = mutableListOf<PassportData>()
  val validator = when (policy) {
    FIELD_PRESENCE -> FieldPresenceValidator
    STRICT -> StrictValidator
  }
  for (entry in input) {
    val passportData = PassportData(entry)
    if (validator.isValid(passportData)) {
      valid += passportData
    }
  }
  return valid
...
interface PassportDataValidator {
  fun isValid(passportData: PassportData): Boolean
