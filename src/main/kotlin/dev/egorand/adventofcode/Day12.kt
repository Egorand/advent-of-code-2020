...
fun Navigator.navigate(instructions: List<String>): Int {
...
private val INSTRUCTION_REGEX = Regex("([NSEWLRF])(\\d+)")

interface Navigator {
  val manhattanDistance: Int
  fun nextInstruction(instruction: String)
}

class ShipNavigator : Navigator {
  private var bearing = EAST
  private var northCoordinate = 0
  private var eastCoordinate = 0

  override val manhattanDistance: Int get() = abs(northCoordinate) + abs(eastCoordinate)

  override fun nextInstruction(instruction: String) {
    val (direction, value) = instruction.parse(INSTRUCTION_REGEX).let { (direction, value) ->
      direction[0].let { if (it == 'F') bearing.name[0] else it } to value.toInt()
    }
    when (direction) {
      'N' -> northCoordinate += value
      'E' -> eastCoordinate += value
      'S' -> northCoordinate -= value
      'W' -> eastCoordinate -= value
      'L' -> bearing -= value
      'R' -> bearing += value
    }
  }
...
class WaypointNavigator : Navigator {
  private var waypointNorthCoordinate = 1
  private var waypointEastCoordinate = 10
  private var shipNorthCoordinate = 0
  private var shipEastCoordinate = 0

  override val manhattanDistance: Int get() = abs(shipNorthCoordinate) + abs(shipEastCoordinate)
  ...
  override fun nextInstruction(instruction: String) {
    ...
    when (direction) {
      'N' -> waypointNorthCoordinate += value
      'E' -> waypointEastCoordinate += value
      'S' -> waypointNorthCoordinate -= value
      'W' -> waypointEastCoordinate -= value
      'L' -> moveWaypointLeft(value)
      'R' -> moveWaypointRight(value)
      'F' -> moveShipToWaypoint(value)
    }
  }

  private fun moveWaypointLeft(degrees: Int) {
    val oldWaypointNorthCoordinate = waypointNorthCoordinate
    val oldWaypointEastCoordinate = waypointEastCoordinate
    when (degrees) {
      90 -> {
        waypointNorthCoordinate = oldWaypointEastCoordinate
        waypointEastCoordinate = -oldWaypointNorthCoordinate
      }
      180 -> {
        waypointNorthCoordinate = -oldWaypointNorthCoordinate
        waypointEastCoordinate = -oldWaypointEastCoordinate
      }
      270 -> {
        waypointNorthCoordinate = -oldWaypointEastCoordinate
        waypointEastCoordinate = oldWaypointNorthCoordinate
      }
      else -> throw AssertionError("Unexpected degrees: $degrees")
    }
  }

  private fun moveWaypointRight(degrees: Int) {
    val oldWaypointNorthCoordinate = waypointNorthCoordinate
    val oldWaypointEastCoordinate = waypointEastCoordinate
    when (degrees) {
      90 -> {
  ...
  private fun moveShipToWaypoint(times: Int) {
    shipNorthCoordinate += waypointNorthCoordinate * times
    shipEastCoordinate += waypointEastCoordinate * times
  }
}
...
enum class Bearing {
  ...
  private val index get() = values().indexOfFirst { it == this }

  operator fun minus(degrees: Int): Bearing {
    val newIndex = (index - degrees / 90).let {
      if (it < 0) values().size + it else it
    }
    return values()[newIndex]
  }

  operator fun plus(degrees: Int): Bearing {
    val newIndex = (index + degrees / 90) % values().size
    return values()[newIndex]
  }
}
