...
fun treesEncountered(map: Array<CharArray>, slope: Slope): Int {
  var treesEncountered = 0
  val tobogganNavigator = TobogganNavigator(map, slope)
  while (!tobogganNavigator.reachedEnd) {
    if (tobogganNavigator.isOnTree) {
      treesEncountered += 1
    }
    tobogganNavigator.move()
  }
...
class TobogganNavigator(
  ...
  val isOnTree get() = map[row][col] == '#'
  val reachedEnd get() = row >= map.size

  fun move() {
    check(!reachedEnd) { "Reached end of map - can't move" }
    row += slope.down
    col = (col + slope.right) % map[0].size
  }

  override fun toString(): String = buildString {
    val columns = min(
      map[0].size,
      map[0].size * (col / map[0].size + 1)
    )
    val currentPosition = row to col
    for (row in map.indices) {
      for (column in 0 until columns) {
        if (row to column == currentPosition) {
          append(if (isOnTree) 'X' else '0')
        } else {
          append(map[row][column % map[row].size])
        }
      }
      append('\n')
    }
