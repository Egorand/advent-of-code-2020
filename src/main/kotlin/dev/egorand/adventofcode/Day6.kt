...
fun countAnswers(declarations: String, mode: Mode): Int {
  val allAnswers = declarations.split("\n\n").map(::groupAnswers)
  return allAnswers.sumBy { (groupSize, groupAnswers) ->
    groupAnswers.count { answers ->
      when (mode) {
        ANYONE_ANSWERED -> answers > 0
        EVERYONE_ANSWERED -> answers == groupSize
      }
    }
  }
}
...
typealias GroupAnswers = Pair<Int, IntArray>

private fun groupAnswers(group: String): GroupAnswers {
  val individualAnswers = group.lines()
  val answerCounts = IntArray(26)
  for (answer in individualAnswers) {
    for (questionId in answer) {
      answerCounts[questionId - 'a'] += 1
    }
  }
  return individualAnswers.size to answerCounts
}
