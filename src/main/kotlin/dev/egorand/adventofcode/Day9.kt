...
fun findFirstInvalidNumber(numbers: LongArray, preambleSize: Int): Long {
  for (i in preambleSize until numbers.size) {
    val seen = mutableSetOf<Long>()
    val number = numbers[i]
    for (j in i - preambleSize until i) {
      if (numbers[j] in seen) break
      seen += number - numbers[j]
      if (j == i - 1) return number
    }
  }
  throw AssertionError("All numbers were valid")
}

fun contiguousRangeThatSumsTo(numbers: LongArray, sum: Long): Long {
  for (i in numbers.indices) {
    var runningSum = 0L
    var smallestInRange = Long.MAX_VALUE
    var largestInRange = Long.MIN_VALUE
    for (j in i until numbers.size) {
      runningSum += numbers[j]
      if (numbers[j] < smallestInRange) smallestInRange = numbers[j]
      if (numbers[j] > largestInRange) largestInRange = numbers[j]
      if (runningSum == sum) {
        return smallestInRange + largestInRange
      } else if (runningSum > sum) {
        break
      }
    }
  }
  throw AssertionError("Unable to find a contiguous range that adds up to $sum")
}
