...
abstract class CrabCombat {
  fun play(cards: String): Long {
    val lines = cards.lines()
    val player1Cards = mutableListOf<Int>()
    val player2Cards = mutableListOf<Int>()
    var lineIndex = 1
    for (i in lineIndex until lines.size) {
      ...
      player1Cards += lines[i].toInt()
    }
    for (i in lineIndex until lines.size) {
      player2Cards += lines[i].toInt()
    }
    return play(Player(1, player1Cards), Player(2, player2Cards)).calculateScore()
  }

  protected abstract fun play(player1: Player, player2: Player): Player

  protected class Player(
    val id: Int,
    cards: Collection<Int>,
  ) {
    val cards = LinkedList(cards)

    fun drawCard(): Int = cards.removeFirst()

    fun wonRound(ownCard: Int, otherCard: Int) {
      cards.addLast(ownCard)
      cards.addLast(otherCard)
    }

    fun lost(): Boolean = cards.isEmpty()

    fun calculateScore(): Long {
      return (cards.size downTo 1L).zip(cards)
        .map { (multiplier, card) -> multiplier * card }
        .sum()
    }
...
object SimpleCrabCombat : CrabCombat() {
  override fun play(player1: Player, player2: Player): Player {
    while (!player1.lost() && !player2.lost()) {
      val player1Card = player1.drawCard()
      val player2Card = player2.drawCard()
      when {
        player1Card > player2Card -> player1.wonRound(player1Card, player2Card)
        player2Card > player1Card -> player2.wonRound(player2Card, player1Card)
        else -> throw AssertionError("Found cards of the same value: $player1Card")
      }
    }
    return if (player1.lost()) player2 else player1
  }
}

class RecursiveCrabCombat : CrabCombat() {
  private val previousRounds = mutableSetOf<Round>()

  override fun play(player1: Player, player2: Player): Player {
    var roundNumber = 1
    while (!player1.lost() && !player2.lost()) {
      val round = Round(player1.cards, player2.cards)
      if (round in previousRounds) return player1
      previousRounds += round

      val player1Card = player1.drawCard()
      val player2Card = player2.drawCard()
      when {
        player1.cards.size >= player1Card && player2.cards.size >= player2Card -> {
          val recursiveRound = RecursiveCrabCombat()
          val subRoundWinner = recursiveRound.play(
            player1 = Player(1, cards = player1.cards.subList(0, player1Card)),
            player2 = Player(2, cards = player2.cards.subList(0, player2Card)),
          )
          when (subRoundWinner.id) {
            1 -> player1.wonRound(player1Card, player2Card)
            2 -> player2.wonRound(player2Card, player1Card)
          }
        }
        player1Card > player2Card -> player1.wonRound(player1Card, player2Card)
        player2Card > player1Card -> player2.wonRound(player2Card, player1Card)
  ...
  data class Round(
    private val player1Cards: Deque<Int>,
    private val player2Cards: Deque<Int>,
  )
}
