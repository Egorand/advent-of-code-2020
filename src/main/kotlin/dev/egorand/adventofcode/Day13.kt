...
fun findEarliestBus(info: String): Int {
  val lines = info.lines()
  val timestamp = lines[0].toDouble()
  val busIds = lines[1].split(',').filter { it != "x" }.map(String::toInt)
  return findEarliestBus(timestamp, busIds)
}

fun findEarliestBus(timestamp: Double, busIds: List<Int>): Int {
  var minMinutesToWait = Int.MAX_VALUE
  var earliestBusId = -1
  for (busId in busIds) {
    val minutesToWait = (busId * (1 - (timestamp / busId % 1))).roundToInt()
    if (minutesToWait < minMinutesToWait) {
      minMinutesToWait = minutesToWait
      earliestBusId = busId
    }
  }
  return earliestBusId * minMinutesToWait
}

fun earliestTimestamp(info: String): BigInteger {
  val lines = info.lines()
  val busIds = mutableListOf<BigInteger>()
  val remainders = mutableListOf<BigInteger>()
  lines[1].split(',').forEachIndexed { index, busId ->
    if (busId != "x") {
      val busId = BigInteger(busId)
      busIds += busId
      remainders += busId - index.toBigInteger()
    }
  }
  return findChineseRemainder(busIds, remainders)
}

private fun findChineseRemainder(
  numbers: List<BigInteger>,
  remainders: List<BigInteger>,
): BigInteger {
  val product = numbers.fold(ONE, BigInteger::times)
  var sum = ZERO
  for (i in numbers.indices) {
    val p = product / numbers[i]
    sum += remainders[i] * multiplicativeInverse(p, numbers[i]) * p
  }
  return sum % product
}

/** @return x where (a * x) % b == 1 */
private fun multiplicativeInverse(
  a: BigInteger,
  b: BigInteger,
): BigInteger {
  if (b == ONE) return ONE
  ...
  return x1 + if (x1 < ZERO) originalB else ZERO
}
