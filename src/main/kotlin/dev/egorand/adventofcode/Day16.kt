 ...
 * is not true of the other three nearby tickets: the values 4, 55, and 12 are are not valid for
 * any field. Adding together all of the invalid values produces your ticket scanning error rate:
 * 4 + 55 + 12 = 71.
 *
...
class TicketScanner private constructor(
  private val yourTicket: IntArray,
  nearbyTickets: List<IntArray>,
  ticketFields: List<TicketField>,
  orderFields: Boolean,
) {
  val scanningErrorRate: Int
  private var orderedFields: Array<TicketField>

  init {
    val (validTickets, scanningErrorRate) = validateTickets(ticketFields, nearbyTickets)
    this.scanningErrorRate = scanningErrorRate

    this.orderedFields = if (orderFields) orderFields(ticketFields, validTickets) else emptyArray()
  }

  private fun validateTickets(
    ticketFields: List<TicketField>,
    tickets: List<IntArray>,
  ): Pair<List<IntArray>, Int> {
    val validTickets = mutableListOf(yourTicket)
    var scanningErrorRate = 0
    for (ticket in tickets) {
      val invalidValue = ticket.firstOrNull { value ->
        ticketFields.none { field -> field.valueInRanges(value) }
      }
      if (invalidValue != null) {
        scanningErrorRate += invalidValue
      } else {
        validTickets += ticket
      }
    }
    return validTickets to scanningErrorRate
  }

  private fun orderFields(
    ticketFields: List<TicketField>,
    validTickets: List<IntArray>,
  ): Array<TicketField> {
    val fieldsByCountOfMatchingIndices = sortedMapOf<Int, Pair<TicketField, MutableList<Int>>>()
    for (field in ticketFields) {
      val matchingIndices = mutableListOf<Int>()
      for (index in ticketFields.indices) {
        if (validTickets.map { it[index] }.all { field.valueInRanges(it) }) {
          matchingIndices += index
        }
      }
      fieldsByCountOfMatchingIndices[matchingIndices.size] = field to matchingIndices
    }
    val orderedTicketFields = ticketFields.toTypedArray()
    val consumedIndices = mutableListOf<Int>()
    for ((field, matchingIndices) in fieldsByCountOfMatchingIndices.values) {
      val index = matchingIndices.first { it !in consumedIndices }
      orderedTicketFields[index] = field
      consumedIndices += index
    }
    return orderedTicketFields
  }

  fun departures(): Long {
    return orderedFields
      .mapIndexedNotNull { index, field -> if (field.name.startsWith("departure")) index else null }
      .map(yourTicket::get)
      .fold(1L, Long::times)
  }

  fun yourTicketValueForField(fieldName: String): Int {
    return yourTicket[orderedFields.indexOfFirst { it.name == fieldName }]
  }

  data class TicketField(
    val name: String,
    private val valueRanges: List<IntRange>,
  ) {
    fun valueInRanges(value: Int): Boolean = valueRanges.any { value in it }
  }

  companion object {
    private val TICKET_FIELD_REGEX = Regex("(.+): (\\d+)-(\\d+) or (\\d+)-(\\d+)")

    operator fun invoke(notes: String, orderFields: Boolean = true): TicketScanner {
      val ticketFields = mutableListOf<TicketField>()
      lateinit var yourTicket: IntArray
      val nearbyTickets = mutableListOf<IntArray>()
      var lineIndex = 0
      ...
      for (i in lineIndex until lines.size) {
        ...
        val (field, range1From, range1To, range2From, range2To) = lines[i].parse(TICKET_FIELD_REGEX)
        ticketFields += TicketField(
          name = field,
          valueRanges = listOf(
            range1From.toInt()..range1To.toInt(),
            range2From.toInt()..range2To.toInt(),
          )
        )
      }
      yourTicket = lines[lineIndex].split(',').map(String::toInt).toIntArray()
      lineIndex += 3
      for (i in lineIndex until lines.size) {
        nearbyTickets += lines[i].split(',').map(String::toInt).toIntArray()
      }
      return TicketScanner(yourTicket, nearbyTickets, ticketFields, orderFields)
    }
  }
}
