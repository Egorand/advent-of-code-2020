...
import dev.egorand.adventofcode.BagGraphDirection.INNER_TO_OUTER
import dev.egorand.adventofcode.BagGraphDirection.OUTER_TO_INNER
...
inline class Bag(val color: String)

typealias ContainedBags = Pair<Int, Bag>

typealias BagGraph = Map<Bag, Set<ContainedBags>>

enum class BagGraphDirection {
  INNER_TO_OUTER,
  OUTER_TO_INNER,
}

fun countBagsThatCanContain(rules: String, bag: Bag): Int {
  val containingBags = mutableSetOf<Bag>()
  val bagsGraph = buildBagGraph(rules, INNER_TO_OUTER)
  with(LinkedList<Bag>()) {
    addLast(bag)
    while (isNotEmpty()) {
      val next = pollFirst()
      val canBeIn = bagsGraph[next] ?: emptySet()
      for ((_, containingBag) in canBeIn) {
        containingBags += containingBag
        addLast(containingBag)
      }
    }
  }
...
fun totalContainedBags(rules: String, bag: Bag): Int {
  fun totalContainedBags(bagGraph: BagGraph, bag: Bag): Int {
    val containedBags = bagGraph[bag] ?: emptySet()
    return 1 + containedBags
      .map { (count, bag) -> count * totalContainedBags(bagGraph, bag) }
      .sum()
  }

  // -1 to not count the outermost bag.
  return totalContainedBags(buildBagGraph(rules, OUTER_TO_INNER), bag) - 1
}

private fun buildBagGraph(rules: String, direction: BagGraphDirection): BagGraph {
  val graph = mutableMapOf<Bag, MutableSet<ContainedBags>>()
  val ruleRegex = Regex("(\\w+ \\w+) bags contain (.+)")
  val bagsWithCountRegex = Regex("(\\d) (\\w+ \\w+).+")
  for (rule in rules.lines()) {
    val (outerColor, innerBags) = rule.parse(ruleRegex)
    for (innerBag in innerBags.split(", ")) {
      if (innerBag == "no other bags.") continue
      val (count, innerColor) = innerBag.parse(bagsWithCountRegex)
      when (direction) {
        INNER_TO_OUTER -> {
          graph.getOrPut(Bag(innerColor), { mutableSetOf() }) += count.toInt() to Bag(outerColor)
        }
        OUTER_TO_INNER -> {
          graph.getOrPut(Bag(outerColor), { mutableSetOf() }) += count.toInt() to Bag(innerColor)
        }
      }
    }
  }
